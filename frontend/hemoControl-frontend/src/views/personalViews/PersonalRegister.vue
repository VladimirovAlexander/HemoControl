...
export default {
  setup() {
    ...
    const register = async () => {
      ...
      try {
        const response = await fetch('https://localhost:7000/api/account/register-staff', {
          method: 'POST',
        ...
        if (response.ok) {
          success.value = 'Сотрудник успешно зарегистрирован!';
          setTimeout(() => router.push('/personal-login'), 1500);
        } else {
